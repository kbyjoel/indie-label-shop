...
- Créer `src/Service/Cart/CartContext.php` : récupère/crée l'`Order` (state=`cart`) persisté en base ; stocke uniquement le `tokenValue` (UUID) en session — pattern natif Sylius.
